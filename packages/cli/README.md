...
`init`은 CSS Modules 또는 Tailwind CSS v4 중 하나를 선택해 `lyrd.json`의 `styling`에 저장한다. 비대화형 환경에서는 `--style css-modules` 또는 `--style tailwind-v4`를 지정한다.
...
첫 번째 `add overlay` 명령도 `--style`을 받으면 `lyrd.json`을 생성할 수 있다. `paths.overlay`는 오버레이 로컬 파일 경로이며, `adapters.overlay`는 생성 템플릿이 사용하는 기반 프리미티브를 기록한다.
...
```text
src/overlays/dialogs/project-settings/
  ProjectSettingsDialog.tsx
  ProjectSettingsDialog.module.css # CSS Modules 선택 시
  index.ts
```
