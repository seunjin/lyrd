...
이 명령은 현재 CLI와 같은 prerelease 채널의 `@lyrd/core@next`와 `@base-ui/react`를 설치하고 다음 파일을 생성한다.
...
~~~bash
pnpm dlx @lyrd/cli@next add dialog project-settings
~~~
...
~~~bash
pnpm dlx @lyrd/cli@next add toast
~~~

이 명령은 앱 소유의 Toast definition, Provider, parallel group, `notify()` helper와 CSS를 생성한다. `AppToastProvider`로 기존 `AppOverlayProvider`를 앱 루트에서 감싸고, 일반 호출부에서는 `notify()` 또는 `notifyWithUndo()`를 사용한다. 표시 개수·timeout·스타일·Undo 동작은 생성된 파일에서 제품에 맞게 수정한다.
