...
- 입력과 결과 타입을 연결하는 `defineOverlay()`와 `overlay.open()`
...
`overlay.open()`과 `overlay.dialog()`은 호출마다 독립 세션을 만든다. 자동 중복 공유는 하지 않으며, 명시적인 upsert 정책은 실제 사례를 검증한 뒤 추가한다. `overlay.dialog(element)`와 `useOverlayDialog()`는 단발성 JSX와 기존 코드 마이그레이션을 위한 escape hatch로 유지한다.
