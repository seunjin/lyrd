...
두 fixture에는 생성 결과를 체크인해 두었다. Vite React는 CSS Modules, Next App Router는 Tailwind CSS v4 출력을 보여준다.

저장소 루트에서 다음 명령을 실행한다. 각 명령은 루트 의존성을 확인하고 로컬 Lyrd 패키지를 빌드한 뒤 fixture 의존성 설치와 개발 서버 시작까지 수행한다.

```bash
# CSS Modules · http://127.0.0.1:4173
pnpm dev:fixture:vite

# Tailwind CSS v4 · http://127.0.0.1:4174/lab
pnpm dev:fixture:next
```

회귀 runner는 체크인된 생성물을 복사본에서 제거한 뒤 CLI로 다시 생성하므로, fixture를 쇼케이스로 유지하면서 실제 generator도 검증한다.
