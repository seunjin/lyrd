...
import { AlertSurface } from '@/overlays/alert/AlertSurface'
import { ConfirmSurface } from '@/overlays/confirm/ConfirmSurface'
...
`lyrd.json`은 CLI가 코드를 생성할 위치, 기반 라이브러리와 `css-modules` 또는 `tailwind-v4` 생성 방식을 기록한다. React 컴포넌트 등록이나 실행 중 정책은 JSON에 넣지 않는다.
...
실제 렌더러 등록은 타입 검사를 받을 수 있는 `OverlayProvider.tsx`에서 관리한다. 이 분리는 다음 장점이 있다.
