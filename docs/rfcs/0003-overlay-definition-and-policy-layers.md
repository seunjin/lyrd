...
또한 현재 항목이 `completeExit()`을 호출하지 않으면 다음 항목이 열리지 않는다. 실제
...
- 입력 props 보관
- 결과 확정과 dismiss
- mount, open, closing, completeExit 수명주기
- 호출이 끝난 뒤의 안전한 정리
...
- danger 버튼이 어떤 색인지
- 비동기 작업의 오류 문구가 무엇인지
- 같은 오버레이를 queue, parallel, openOrUpdate 중 무엇으로 실행할지
...
overlay.openOrUpdate(definition, identity, props)
```

`open()`은 기본적으로 새로운 세션을 만든다. `openOrUpdate()`만 기존 세션을 재사용하고 props를
갱신한다. 이렇게 하면 중복 처리 의도가 호출부에 드러난다.

두 호출은 모두 실제 Promise를 확장한 `OverlayHandle<Input, Result>`를 반환한다.

```ts
const handle = overlay.open(definition, input)
...
Handle은 자신이 가리키는 활성 세션만 제어한다. 종료 뒤 `update()`와 `dismiss()`는 `false`를
반환하며 새 세션을 만들지 않는다. 여러 호출부가 업무 identity로 같은 활성 세션을 찾아야
할 때만 `openOrUpdate()`를 다시 호출한다.

첫 `openOrUpdate` 계약은 다음과 같이 제한한다.
...
- `identity`는 필수 문자열이다.
- 같은 definition 객체와 identity를 가진 활성 `openOrUpdate` 세션만 재사용한다.
- 재사용하면 최초 Handle과 렌더러 인스턴스를 유지하고 input을 최신 값으로 바꾼다.
...
각각 mount, open, closing, completeExit 상태와 Promise를 가지며 서로의 닫힘을 기다리지
않는다. `dismissAll()`은 기본 queue와 parallel session을 같은 reason으로 정리한다.
`openOrUpdate()`로 만든 활성 세션의 group은 중간에 바꿀 수 없다. 실행 위치를 바꾸려면 기존
...
Lyrd는 `completeExit()`을 유지한다. 실제 UI 프리미티브의 애니메이션 완료 신호와
연결할 수 있기 때문에 고정 millisecond delay보다 정확하다.

다만 연결 누락의 부담을 낮추기 위해 다음을 검토한다.

- 애니메이션이 없는 렌더러용 `completeExit()` 자동 처리
...
- resolve, dismiss, requestDismiss, completeExit을 연결한다.
- alert, confirm, dialog의 기존 테스트를 유지한다.

### 3단계: 자동 dedupe 정리

- `dialog()`의 `element.type + key` 자동 공유를 제거한다.
- 명시적 identity가 필요한 실제 사례를 기준으로 `openOrUpdate` 또는 `dedupeKey`를 설계한다.
...
- 같은 definition과 identity를 openOrUpdate하면 Handle을 유지하며 최신 input을 렌더링한다.
- 일반 open, 다른 identity, settle된 세션은 openOrUpdate와 공유하지 않는다.
...
- 현재 queue와 `completeExit()` 수명주기 연결
...
- `overlay.openOrUpdate(definition, identity, input, options)` 공개 API
...
- `overlay.open()`과 `overlay.openOrUpdate()`의 선택적 `group` option
- 기본 queue snapshot과 독립된 다중 definition snapshot
- parallel session별 resolve, dismiss, requestDismiss, completeExit
- parallel session과 modal queue를 함께 정리하는 `dismissAll`
- 활성 openOrUpdate session의 group 불변 규칙
...
5. `open()`과 `dialog()`은 자동 dedupe하지 않는다. 진행률처럼 동일 작업을 갱신할 때만
   필수 identity를 받는 `openOrUpdate()`로 활성 세션을 명시적으로 재사용한다.
6. group을 생략하면 안전한 기본 queue를 사용한다. Toast처럼 실제 동시성이 필요한 호출만
   `defineOverlayGroup({ strategy: 'parallel' })`로 명시적으로 분리한다.

2026-07-16 stable API 감사에서 다음을 추가로 확정했다.

7. 일반 호출자가 사용하는 `alert`, `confirm`, `open`, `openOrUpdate`, `dismissAll`은 Application
   API로, definition과 UI primitive를 연결하는 session method는 Renderer API로 문서에서
   분리한다.
8. 외부 dismiss 시도는 `requestDismiss`, exit lifecycle 완료는 `completeExit`으로 표현한다.
   호출 옵션은 session의 명령형 `dismiss()`와 구분되는 `dismissPolicy` 이름을 사용한다.
9. Overlay group은 strategy wrapper가 아니라 같은 실행 전략과 상태 공간을 공유하는
   coordination boundary다. Core는 group identity마다 별도 runtime session collection을
   유지한다. 별도 queue group과 group 단위 제어 API는 실제 사례가 생길 때 설계한다.
10. DB 용어인 `upsert()`는 동작이 드러나는 `openOrUpdate()`로 바꾼다. 이 API는 같은
    definition과 identity의 활성 세션이 있으면 갱신하고, 없으면 새 세션을 만든다.
11. `open()`과 `openOrUpdate()`는 직접 `await`할 수 있는 `OverlayHandle`을 반환한다. Handle을
    보관한 호출자는 `update()`와 `dismiss()`로 그 세션만 제어할 수 있으며, 종료된 Handle은
    새 세션을 만들지 않는다.
